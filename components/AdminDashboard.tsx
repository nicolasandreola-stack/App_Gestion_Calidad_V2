...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout, currentUser, onOpenSearch, initialTab = 'asignaciones', showTokenHelpExternal, onCloseTokenHelpExternal }) => {
    ...
    const handleRestoreTask = (task: Task) => {
        ...
        console.log("Tarea restaurada al tablero del usuario.");
    };
    ...
    const handleBulkAddProjects = async (newProjList: ProjectTask[]) => {
        const nextList = [...globalProjects, ...newProjList];
        setGlobalProjects(nextList);
        localStorage.setItem('v25_global_projects', JSON.stringify(nextList));
        await syncProjectsCloud(nextList);
    };

    const handleBulkUpsertProjects = async (upsertList: ProjectTask[]) => {
        let nextList = [...globalProjects];
        upsertList.forEach(imp => {
           const existingIdx = nextList.findIndex(p => p.id === imp.id);
           if (existingIdx >= 0) {
              nextList[existingIdx] = imp; // Actualiza
           } else {
              nextList.push(imp); // Agrega
           }
        });
        setGlobalProjects(nextList);
        localStorage.setItem('v25_global_projects', JSON.stringify(nextList));
    ...
    const handleReorderProjects = async (reorderedProjects: ProjectTask[]) => {
        setGlobalProjects(reorderedProjects);
        localStorage.setItem('v25_global_projects', JSON.stringify(reorderedProjects));
        await syncProjectsCloud(reorderedProjects, projectObservations);
    };
    ...
    return (
        <div className="flex flex-col h-full bg-bgBody font-sans overflow-hidden">
            ...
            {/* ADMIN TOPBAR: Solo visible en vistas de equipo (no gantt) */}
            {adminViewTab !== 'gantt' && (
